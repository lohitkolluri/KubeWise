// Package kwtable is a fork of charm.land/bubbles/v2/table with per-cell
// selected-row styling so highlight backgrounds span every column.
package kwtable
...
func (m *Model) renderRow(r int) string {
	...
	for i, value := range m.rows[r] {
		...
		cellContent := style.Render(ansi.Truncate(value, m.cols[i].Width, "…"))
		cellStyle := m.styles.Cell
		if r == m.cursor && m.focus {
			cellStyle = m.selectedCellStyle()
		}
...
func (m Model) selectedCellStyle() lipgloss.Style {
	sel := m.styles.Selected
	cell := m.styles.Cell
	out := cell.
		Foreground(sel.GetForeground()).
		Background(sel.GetBackground()).
		Bold(sel.GetBold()).
		Italic(sel.GetItalic()).
		Blink(sel.GetBlink()).
		Reverse(sel.GetReverse()).
		Faint(sel.GetFaint())
	if sel.GetUnderline() {
		out = out.Underline(true).UnderlineStyle(sel.GetUnderlineStyle()).UnderlineSpaces(sel.GetUnderlineSpaces())
	}
	if sel.GetStrikethrough() {
		out = out.Strikethrough(sel.GetStrikethrough()).StrikethroughSpaces(sel.GetStrikethroughSpaces())
	}
	return out
}
