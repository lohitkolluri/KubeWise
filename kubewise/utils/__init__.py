...
def format_metric_value(metric_name: str, value: float) -> str:
    ...
    if (
        metric_name.endswith('_pct') or 
        metric_name.endswith('_percent') or 
        metric_name.endswith('_percentage') or
    ...
    if (
        metric_name.endswith('_seconds') or 
        metric_name.endswith('_ms') or 
        'latency' in metric_name or 
        'duration' in metric_name
    ):
        if value < 1:
    ...
    if (
        metric_name.endswith('_bytes') or 
        'memory' in metric_name or 
        'disk' in metric_name or 
        'storage' in metric_name
    ):
        if value < 1024:
            return f"{value:.0f}B"
