...
async def create_gemini_agent() -> Optional[Agent]:
    """Create and configure the Gemini agent for AI-powered remediation."""
    if not settings.gemini_api_key or settings.gemini_api_key.get_secret_value() == "changeme":
...
async def create_prometheus_fetcher(app_context: AppContext) -> Optional[PrometheusFetcher]:
    ...
    if app_context.metric_queue is None or app_context.http_client is None:
        logger.error("Cannot create Prometheus fetcher: Required dependencies not available")
        return None
    ...
    try:
        ...
        try:
            ...
            if not prometheus_url:
                logger.warning("Prometheus URL not configured. Metrics collection disabled.")
                return None
                
            metrics_queries = settings.prom_queries
        except AttributeError as attr_err:
            logger.warning(f"Prometheus configuration is incomplete: {attr_err}. Using default values.")
            prometheus_url = "http://localhost:9090"
            metrics_queries = DEFAULT_PROM_QUERIES if "DEFAULT_PROM_QUERIES" in globals() else {}
...
async def create_planner_dependencies(
    ...
    """Create planner dependencies container with database and K8s client."""
    if app_context.db is None or app_context.k8s_api_client is None:
        raise ValueError("Database and Kubernetes client must be initialized")
...
async def initialize_app_context() -> AppContext:
    ...
    try:
        ...
        logger.exception(f"FATAL: Failed to connect to MongoDB after multiple retries: {e}")
    ...
    if app_context.db is not None:
        try:
            ...
            logger.exception(f"Failed to initialize anomaly detector: {detector_init_err}")
    ...
    if app_context.db is not None and app_context.k8s_api_client is not None:
        try:
            ...
            logger.exception(f"Failed to initialize Kubernetes event watcher: {watcher_init_err}")
            app_context.k8s_event_watcher = None
            logger.error("Kubernetes event watcher failed to initialize. Event watching will not work.")
