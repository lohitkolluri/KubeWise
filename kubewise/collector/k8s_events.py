...
def parse_k8s_event(raw_event: dict) -> Optional[KubernetesEvent]:
    ...
    try:
        ...
        raw_event["event_id"] = metadata.get(
            "uid", f"unknown-{datetime.datetime.now().isoformat()}"
        )
        ...
        if (
            not parsed_event.involved_object_name
...
class KubernetesEventWatcher:
    ...
    async def start(self) -> None:
        ...
        self._watch_tasks.add(
            asyncio.create_task(
                self._event_watch_loop(), name="kubernetes_event_watcher"
            )
        )
        self._health_check_task = asyncio.create_task(
            self._health_check_loop(), name="kubernetes_event_watcher_health"
        )
        self._resource_state_task = asyncio.create_task(
            self._resource_state_loop(), name="kubernetes_resource_state_checker"
        )

        logger.info(
            f"Kubernetes event watcher started successfully with resource_version: {self._resource_version or 'latest'}"
        )
    ...
    async def _event_watch_loop(self) -> None:
        ...
        try:
            ...
            if self._resource_version:
                logger.info(
                    f"Resuming event watch from resourceVersion {self._resource_version}"
                )
            else:
                logger.info(
                    "Starting event watch from current state (no stored resourceVersion)"
                )
    ...
    async def _watch_stream_with_backoff(
        ...
        for namespace in namespaces:
            ...
            logger.info(
                f"Starting event watch for {name_str} from resourceVersion {self._resource_version or 'current'}"
            )
    ...
    async def _process_event(self, event_obj: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if event_type == "ERROR":
                ...
                if reason == "Expired":
                    logger.warning(
                        "Kubernetes watch resource version expired. Restarting with latest."
                    )
                ...
                if (
                    hasattr(raw_event, "metadata")
                    and raw_event.metadata
                    and hasattr(raw_event.metadata, "resource_version")
                ):
                    new_rv = raw_event.metadata.resource_version
                    if new_rv != self._resource_version:
                        logger.debug(
                            f"Received watch bookmark. Updated resource_version to {new_rv}"
                        )
                    ...
                    logger.debug(
                        "Received BOOKMARK event without resource_version in metadata, ignoring."
                    )
                ...
                try:
                    ...
                    if not raw_event_dict:
                        logger.warning(
                            f"Could not convert event to dictionary: {type(raw_event)}"
                        )
                        return

                    # Update resource state tracking for involved object
                    if (
                        hasattr(raw_event, "involved_object")
                        and raw_event.involved_object
                    ):
                        # It's a CoreV1Event object
                        involved_object_dict = (
                            raw_event.involved_object.to_dict()
                            if hasattr(raw_event.involved_object, "to_dict")
                            else {}
                        )
                        kind = getattr(raw_event.involved_object, "kind", None)
                        namespace = getattr(
                            raw_event.involved_object, "namespace", None
                        )
                    ...
                    if not kind or not name:
                        logger.debug(
                            f"Event missing kind ({kind}) or name ({name}): {reason} - {message}"
                        )
                    ...
                    try:
                        if self._should_skip_event_for_healthy_resource(raw_event):
                            logger.debug(
                                f"Skipping event for healthy resource: {reason} - {message}"
                            )
                            return
                    except Exception as e:
                        logger.warning(
                            f"Error checking if event should be skipped: {e}"
                        )
                    ...
                    try:
                        ...
                        if parsed_event:
                            ...
                            if parsed_event.involved_object_name:
                                ...
                                logger.debug(
                                    f"Raw event data: {str(raw_event_dict)[:500]}"
                                )
                        ...
                        logger.error(
                            f"Error parsing event: {e}, event data: {raw_event_dict.get('reason')} - {raw_event_dict.get('message', '')[:100]}"
                        )
                except Exception as e:
                ...
                logger.warning(
                    f"Unknown event type received from watch stream: {event_type}"
                )
    ...
    async def _health_check_loop(self) -> None:
        ...
        logger.info(
            f"Starting Kubernetes event watcher health check loop (every {HEALTH_CHECK_INTERVAL}s)"
        )
            ...
            try:
                ...
                try:
                    ...
                    if not hasattr(self, "_last_watch_success"):
                        self._last_watch_success = time.monotonic()
                        logger.warning(
                            "Initialized missing _last_watch_success attribute"
                        )

                    # Now safely check for recent activity
                    recent_activity = (
                        self._last_watch_success is not None
                        and time.monotonic() - self._last_watch_success
                        < HEALTH_CHECK_INTERVAL
                    )

                    if recent_activity:
                        logger.debug(
                            "Skipping health check, recent successful event processing"
                        )
                        continue
                    ...
                    self._last_watch_success = time.monotonic()
                    logger.warning(
                        "Reinitialized missing _last_watch_success attribute"
                    )
                    continue
                ...
                if not any(task and not task.done() for task in self._watch_tasks):
                    if any(task and task.exception() for task in self._watch_tasks):
                        logger.error(
                            f"Watch task failed with exception: {[task.exception() for task in self._watch_tasks if task.exception()]}"
                        )

                    logger.warning(
                        "Watch task is not running. Restarting event watcher."
                    )
    ...
    async def _get_last_resource_version(self) -> str:
        ...
        logger.info(
            "Starting watch from latest events to avoid resource version expiration"
        )
        return ""
    ...
    async def _resource_state_loop(self) -> None:
        ...
        try:
            ...
            self._logger.info(
                f"Starting Kubernetes resource state checker loop (every {check_interval}s)"
            )
        ...
        try:
            ...
            for namespace, pod_names in pods_by_namespace.items():
                try:
                    ...
                    pod_status = {
                        pod.metadata.name: pod.status for pod in pod_list.items
                    }
                    ...
                    for pod_name in pod_names:
                        ...
                        if pod_name in pod_status:
                            ...
                            container_statuses = (
                                getattr(status, "container_statuses", []) or []
                            )
                            ...
                            is_healthy = (
                                phase == "Running"
                                and all(
                                    getattr(cs, "ready", False)
                                    for cs in container_statuses
                                )
                                and not any(
                                    getattr(getattr(cs, "state", None), "waiting", None)
                                    is not None
                                    and getattr(
                                        getattr(cs, "state", None).waiting, "reason", ""
                                    )
                                    in [
                                        "CrashLoopBackOff",
                                        "ImagePullBackOff",
    ...
    async def _check_nodes_health(self, nodes: List[Tuple[str, str]]):
        ...
        try:
            ...
            # Create a lookup for quick matching
            node_statuses = {
                node.metadata.name: node.status for node in node_list.items
            }
            ...
            for node_name in node_names:
                ...
                if node_name in node_statuses:
                    ...
                    memory_pressure = next(
                        (c for c in conditions if c.type == "MemoryPressure"), None
                    )
                    disk_pressure = next(
                        (c for c in conditions if c.type == "DiskPressure"), None
                    )
                    pid_pressure = next(
                        (c for c in conditions if c.type == "PIDPressure"), None
                    )
                    ...
                    is_healthy = (
                        ...
                        and (
                            memory_pressure is None or memory_pressure.status != "True"
                        )
                        and (disk_pressure is None or disk_pressure.status != "True")
                        and (pid_pressure is None or pid_pressure.status != "True")
                    )
                ...
                try:
                    ...
                    for deployment_name in deployment_names:
                        ...
                        if deployment_name in deployment_statuses:
                            ...
                            is_healthy = (
                                (desired == 0 or available > 0)
                                and (
                                    available >= desired * 0.5
                                )  # At least 50% of desired replicas
                            )
                    ...
                    self._logger.exception(
                        f"Error checking deployments in namespace {namespace}: {e}"
                    )
            ...
            for namespace, statefulset_names in statefulsets_by_namespace.items():
                try:
                    ...
                    statefulset_list = await asyncio.to_thread(
                        apps_api.list_namespaced_stateful_set, namespace=namespace
                    )
                    ...
                    self._logger.exception(
                        f"Error checking statefulsets in namespace {namespace}: {e}"
                    )
            ...
            for namespace, daemonset_names in daemonsets_by_namespace.items():
                try:
                    ...
                    daemonset_list = await asyncio.to_thread(
                        apps_api.list_namespaced_daemon_set, namespace=namespace
                    )
                    ...
                    for daemonset_name in daemonset_names:
                        ...
                        if daemonset_name in daemonset_statuses:
                            ...
                            # 2. No failed pods
                            current = (
                                getattr(status, "current_number_scheduled", 0) or 0
                            )
                            desired = (
                                getattr(status, "desired_number_scheduled", 0) or 0
                            )
                    ...
                    self._logger.exception(
                        f"Error checking daemonsets in namespace {namespace}: {e}"
                    )
    ...
    async def _share_resource_states(self):
        ...
        try:
            ...
            self._logger.debug(
                f"Tracking health status of {len(self._resource_states)} resources"
            )
    ...
    async def _get_pods(self, namespace="default"):
        ...
        try:
            ...
            if hasattr(pod_list, "items"):
                ...
                self._logger.error(
                    f"Pod list response has no 'items' attribute: {type(pod_list)}"
                )
    ...
    async def _get_nodes(self):
        ...
        try:
            ...
            if hasattr(node_list, "items"):
                ...
                self._logger.error(
                    f"Node list response has no 'items' attribute: {type(node_list)}"
                )
                # Return an empty list as fallback
    ...
    def _update_resource_state_from_event(self, event: dict):
        ...
        try:
            ...
            if hasattr(event, "to_dict"):
                ...
                involved_object = (
                    event.involved_object if hasattr(event, "involved_object") else None
                )
            else:
            ...
            if isinstance(involved_object, dict):
                ...
                namespace = (
                    getattr(involved_object, "namespace", "") if involved_object else ""
                )
    ...
    def _should_skip_event_for_healthy_resource(
        self, raw_event: Union[Dict[str, Any], object]
    ) -> bool:
        """
        ...
        try:
            ...
            is_core_v1_event = hasattr(raw_event, "type") and hasattr(
                raw_event, "involved_object"
            )
            ...
            if is_core_v1_event:
                involved_object = (
                    raw_event.involved_object
                    if hasattr(raw_event, "involved_object")
                    else None
                )
            ...
            if not kind or not name:
                logger.debug(
                    f"Skipping health check for event with missing kind or name: {event_type} - {raw_event.get('reason') if not is_core_v1_event else getattr(raw_event, 'reason', 'Unknown')}"
                )
                return False
            ...
            if is_healthy and checked_at:
                ...
                if event_timestamp and event_timestamp < checked_at:
                    reason = (
                        getattr(raw_event, "reason", None)
                        if is_core_v1_event
                        else raw_event.get("reason", "Unknown")
                    )
                    logger.debug(
                        f"Skipping event {reason} for healthy resource {resource_key}. Event from {event_timestamp}, resource checked at {checked_at}"
                    )
                    return True
...
async def watch_kubernetes_events(
    db: motor.motor_asyncio.AsyncIOMotorDatabase,
) -> AsyncGenerator[KubernetesEvent, None]:
    """
...
if __name__ == "__main__":
    ...
    async def test_legacy_api():
        ...
        try:
            ...
            async for event in watch_kubernetes_events(test_db):
                ...
                print(
                    f"  Object: {event.involved_object_kind}/{event.involved_object_namespace}/{event.involved_object_name}"
                )
    ...
    async def test_watcher_class():
        ...
        try:
            ...
            logger.info(
                f"Watcher stats: processed={watcher._watch_restarts}, restarts={watcher._watch_restarts}"
            )
