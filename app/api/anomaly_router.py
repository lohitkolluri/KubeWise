...
from app.core.dependencies.service_factory import (
    get_anomaly_event_service,
    get_gemini_service,
)
...
async def analyze_anomaly(
    ...
    try:
        ...
        if gemini_service and gemini_service.is_api_key_missing():
            ...
            raise HTTPException(
                status_code=503, detail="Gemini service is not configured or available."
            )
        ...
        anomaly_context = {
            ...
            "metrics_snapshot": (
                event.metric_snapshot if isinstance(event.metric_snapshot, list) else []
            ),  # Ensure it's a list
            "anomaly_score": event.anomaly_score,
            "timestamp": event.detection_timestamp.isoformat(),
            "prediction_data": (
                event.prediction_data if event.prediction_data else None
            ),  # Handle null case
        }
        ...
        if not api_key_missing:
            ...
            # Wrap the single anomaly_context in a list since batch_process_anomaly expects a list
            batch_result = await gemini_service.batch_process_anomaly([anomaly_context])

            if batch_result and "error" not in batch_result:
                # Extract info from batch processing - get the first result since we only sent one item
                if isinstance(batch_result, list) and batch_result:
                    batch_result = batch_result[0]  # Get the first result
                ...
                logger.info(
                    "Batch processing failed, falling back to separate API calls"
                )
                analysis_res = await gemini_service.analyze_anomaly(anomaly_context)
                remediation_res = await gemini_service.suggest_remediation(
                    anomaly_context
                )
                ...
                if (
                    remediation_res
                    and "error" not in remediation_res
                    and "steps" in remediation_res
                ):
        ...
        update_data = {
            "ai_analysis": (
                ...
                else event.ai_analysis
            ),
            "suggested_remediation_commands": (
                suggestions if suggestions else event.suggested_remediation_commands
            ),
        }

        await event_service.update_event(anomaly_id, AnomalyEventUpdate(**update_data))

        # For the response, provide clear feedback on API key status
        if api_key_missing:
            response_message = (
                "Anomaly analysis not available: Gemini API Key is not configured"
            )
        else:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to analyze anomaly: {str(e)}"
        )
